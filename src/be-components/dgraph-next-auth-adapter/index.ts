...
interface ExtendedAdapterUser extends AdapterUser {
  // field comes with linkedIn provider.
  // If the user email is verified by LinkedIn, I would trust it.
  email_verified?: string;
  issuer?: string;
}

export function DgraphAdapter(client: DgraphClientParams, options?: DgraphAdapterOptions): Adapter {
  ...
  return {
    async createUser(user: ExtendedAdapterUser) {
      ...
      // Dgraph does not accept user generated id
      const { id, email_verified, issuer, ...userWithoutId } = user;

      const toSave: Omit<AdapterUser, 'id'> = {
      ...
      // if the token issuer is linkedin and email is verfied, consider it verified here as well.
      if (issuer === issuers.linkedin) {
        const date: Date = new Date();
        toSave.emailVerified = email_verified === 'true' ? date : null;
      }
        ...
        /* GraphQL */ `
          mutation UseVerificationToken($token: String!, $identifier: String!) {
            deleteVerificationToken(
              filter: { and: [{ token: { eq: $token } }, { identifier: { eq: $identifier } }] }
            ) {
              verificationToken {
